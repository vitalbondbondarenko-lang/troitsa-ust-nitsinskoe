...
function NotesGuidePage({ openRequest }) {
  return (
    <><PageHero eyebrow="Богослужения" title="Как подать записку" lead="Приход принимает поминовения о здравии и об упокоении. Онлайн-форма пока показана для согласования." breadcrumbs={[{ label: "Богослужения" }, { label: "Как подать записку" }]} /><section className="guide-grid section-wrap"><article><span>01</span><h2>Выберите поминовение</h2><p>Отдельно укажите: «О здравии» или «Об упокоении». Другие виды треб уточняйте по телефону.</p></article><article><span>02</span><h2>Напишите имена</h2><p>Указывайте имена, данные в Крещении, каждое с новой строки. Записки подаются за крещёных православных.</p></article><article><span>03</span><h2>Оставьте контакт</h2><p>Телефон понадобится, чтобы приход подтвердил получение и уточнил ближайшую службу.</p></article><article><span>04</span><h2>Дождитесь подтверждения</h2><p>Ожидаемый срок — ближайшая суббота или праздничная служба; порядок ещё утверждается настоятелем.</p></article></section><section className="section-wrap vk-callout"><div><p className="eyebrow">Уже сейчас</p><h2>Написать в сообщество прихода</h2><p>В сообществе VK указано, что имена для поминовения можно передать личным сообщением группе.</p></div><SourceLink href={VK_COMMUNITY_URL}>Открыть сообщество VK</SourceLink></section><div className="center-action section-wrap"><button className="button button--gold" type="button" onClick={() => openRequest("note")}>Открыть демо-форму</button></div></>
  );
}
